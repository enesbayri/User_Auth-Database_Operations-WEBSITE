...
<div class="container">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form method="post" action="{{route('uyeler.update',$uyem->id)}}">
                @method('PATCH')
                @csrf
                <div class="form-group">
                    @csrf
                    <label for="adSoyad">Ad Soyad</label><br>
                    <input type="text" name="adSoyad" class="form-control" value="{{$uyem->adSoyad}}">
                </div>

                <div class="form-group">
                    @csrf
                    <label for="tc">TC kimlik numaranız: <comm>(zorunlu değildir)</comm></label><br>
                    <input type="number" name="tc" class="form-control" value="{{$uyem->tc}}">
                </div>

                <div class="form-group">
                    @csrf
                    <label for="kullaniciAdi">Kullanıcı Adı</label><br>
                    <input type="text" name="kullaniciAdi" class="form-control" value="{{$uyem->kullaniciAdi}}">
                </div>

                <div class="form-group">
                    @csrf
                    <label for="email">E-mail</label><br>
                    <input type="email" name="email" class="form-control" value="{{$uyem->email}}">
                </div>

                <div class="form-group">
                    @csrf
                    <label for="sifre">Sifre</label><br>
                    <input type="password" name="sifre" class="form-control" value="{{$uyem->sifre}}">
                </div>
